...
var splashScreenMap = {
  "SplashScreen":  [ "620x300",	 "868x420", "1156x540" ],
}
    ...
    if (originalManifest.icons && originalManifest.icons.length) {
      for (var i = 0; i < originalManifest.icons.length; i++) {
        ...
        if (isValidIconFormat(icon, validIconFormats)) {
          // process logos
          for (var elementName in iconMap) {
            var elementSizes = iconMap[elementName];
            var index = elementSizes.indexOf(icon.sizes.toLowerCase());
            if (index >= 0) {
              icons[elementSizes[index]] = { 'url': icon.src, 'fileName': elementName + '.scale-' + scaleMap[index] + '.png' };
            }
          }

          // process splashscreens
          for (var elementName in splashScreenMap) {
            var elementSizes = splashScreenMap[elementName];
            var index = elementSizes.indexOf(icon.sizes.toLowerCase());
            if (index >= 0) {
              icons[elementSizes[index]] = { 'url': icon.src, 'fileName': elementName + '.scale-' + scaleMap[index] + '.png' };
            }
