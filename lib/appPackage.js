...
function invokeAutoPublish(packagePath, username, email, appName, callback) {
  var deferred = Q.defer();
  var formData = {
    username : username,
    email : email,
...
var autoPublish = function (packagePath, username, email, appName, callback) {
  log.debug('Invoking the AutoPublish service to publish .web package');
  return invokeAutoPublish(packagePath, username, email, appName)
  .nodeify(callback);
}
