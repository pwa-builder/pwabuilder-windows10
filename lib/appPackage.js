...
function invokeAutoPublish(packagePath, name, email, appName, callback) {
  var deferred = Q.defer();
  var formData = {
    name : name,
    email : email,
...
var autoPublish = function (packagePath, name, email, appName, callback) {
  log.debug('Invoking the AutoPublish service at ' + publishEndpoint + ' to publish .web package');
  return invokeAutoPublish(packagePath, name, email, appName)
  .nodeify(callback);
}
