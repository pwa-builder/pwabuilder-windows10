...
function Platform (packageName, platforms) {
  ...
  self.create = function (w3cManifestInfo, rootDir, options, callback) {
    ...
    return manifest.convertFromBase(w3cManifestInfo)
      ...
      .then(function () {
        self.debug('Saving the original W3C manifest to the app folder...');
        var w3cManifestFilePath = path.join(platformDir, 'w3cmanifest.json');
  ...
  self.package = function (projectDir, options, callback) {
    ...
    var packagePath = dotWeb ? path.join(outputPath, 'windows.web') : path.join(outputPath, 'windows.appx');

    return fileTools.mkdirp(outputPath).then(function () {
      if (dotWeb) {
        return appPackage.makeWeb(directory, packagePath);
      } else {
