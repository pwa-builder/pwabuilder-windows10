...
function Platform (packageName, platforms) {
  ...
  self.package = function (projectDir, options, callback) {
    ...
    function manifestCallback(err, manifestInfo) {
      ...
      return fileTools.mkdirp(outputPath).then(function () {
        if (dotWeb) {
          ...
          var askUsername = function() {
            var defer = Q.defer();
            rl.question('Please enter your username: ', (username) => { defer.resolve(username) });

            return defer.promise;
          }

          var askEmailAddress = function(username) {
            var defer = Q.defer();
            
            rl.question('Please enter your email address: ', (email) => { 
              rl.close(); 
              defer.resolve([username, email]) 
            });
          ...
          return askUsername()
            .then(askEmailAddress)
            .spread(function(username, email) {
              self.info('Username: ' + username);
              self.info('Email: ' + email);
              return appPackage.makeWeb(directory, outputPath, appName)
                .then(function (packagePath) {
                  self.info('Invoking the AutoPublish service to publish .web package');
                  return appPackage.autoPublish(packagePath, username, email, appName);
              });
          });
        } else {
    ...
    manifestTools.getManifestFromFile(w3cManifestFilePath, lib.constants.BASE_MANIFEST_FORMAT, manifestCallback);
